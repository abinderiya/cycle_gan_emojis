...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = torch.sum((D_X(images_X) - 1)**2)/opts.batch_size
        D_Y_loss = torch.sum((D_Y(images_Y) - 1)**2)/opts.batch_size
        ...
        g_loss = torch.sum((D_X(fake_X) - 1)**2)/opts.batch_size
        ...
        g_loss = torch.sum((D_Y(fake_Y) - 1)**2)/opts.batch_size

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = torch.sum((reconstructed_X - images_X)**2)/opts.batch_size
            g_loss += cycle_consistency_loss
